...
export const getallProjectIncharge = createAsyncThunk("admin/getAllProjectIncharge", async (_, {rejectWithValue, getState}) => {
  ...
  const response = await axios.get("https://mepl-erp.co.in/superadmin/get-allprojectIncharge",{
  ...
  const response = await axios.post("https://mepl-erp.co.in/superadmin/add-projectIncharge", payload,{
  ...
  export const changeRole = createAsyncThunk("admin/changeRole", async (payload, {rejectWithValue, getState}) => {
    try{
        const token= getState().auth.token
  const response = await axios.post("https://mepl-erp.co.in/superadmin/changeRole", payload,{
    headers: {Authorization: `Bearer ${token}`},
  });
  console.log(response);
